...
public interface ParkingTicketsRepositoryData extends JpaRepository<ParkingTicketsData, UUID> {
  ...
  @Query(
      "UPDATE ParkingTicketsData p SET  p.totalToCharge = ?2, p.exitTime = CURRENT_TIMESTAMP WHERE p.id ="
          + " ?1")
  ...
  @Query(
      "UPDATE ParkingTicketsData p SET p.status = 'CLOSED', p.totalToCharge = ?2, p.closedAt ="
          + "CURRENT_TIMESTAMP WHERE p.id = ?1")
  void closeTicket(UUID ticketId, BigDecimal amountPaid);
}
